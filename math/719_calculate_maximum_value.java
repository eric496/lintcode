...
Thought process:
    Method 1:
        Create a current maximum variable to store the max so far. Iterate the array, the principle to get the global max is 
            1) By adding the current digit to the max so far if the current digit and the max so far are both either 0 or 1 (otherwise, the result will be smaller by multiplication).
    ...
    Method 2:
        Iterate the string, if the current number is 0 or 1, addition always yields the max result.
